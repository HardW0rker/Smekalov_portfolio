    ...
    async answer(answer,num){
      ...
      if(answer){
        ...
        var post = await axios
          .post(path.path_edit_point(),{
            point_id: this.data.points[num].point_id,
            date:`${date.getFullYear()}-${('00'+month).slice(-2)}-${date.getDate()}`
          })
        if(post.status){
          new_data.points[num].date = `${date.getDate()}.${('00'+month).slice(-2)}.${date.getFullYear()}`;
          new_data.points[num].status = true;
        }
      }
    }
